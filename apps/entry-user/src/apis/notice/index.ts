...
export const useUpdateNotice = () => {
  return useMutation({
    mutationFn: async(data: INoticeUpdateType) => {
      const response = await AdmissionAdminInstance.patch(`${path}/${data.noticeId}`, data.data);
...
export const useCreateNotice = () => {
  return useMutation({
    mutationFn: async(data: INoticeCreateType) => {
      const response = await AdmissionAdminInstance.post(`${path}`, data);
      return response.data;
    }, 
    onSuccess: () => {
...
export const useGetAllNotice = (type: 'GUIDE' | 'NOTICE') => {
  return useQuery({
    ...
    queryFn: async () => {
      const { data } = await AdmissionAdminInstance.get(`${path}`, {
        params: { type },
      });
      return data;
    },
...
export const useGetDetailNotice = (noticeId: string) => {
  return useQuery({
    ...
    queryFn: async () => {
      const { data } = await AdmissionAdminInstance.get(`${path}/${noticeId}`);
      return data;
    },
  });
};